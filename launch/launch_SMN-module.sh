...
bams=$(<${WRKDIR}/SMNmodule/${cohort_name}_sample_bams tr '\n' ',') 
bams=$(echo "$bams" | sed 's/,/","/g' | sed 's/^/"/' | sed 's/,"$//')

IDs=$(<${WRKDIR}/SMNmodule/${cohort_name}_sample_IDs tr '\n' ',') 
IDs=$(echo "$IDs" | sed 's/,/","/g' | sed 's/^/"/' | sed 's/,"$//')

    cat <<EOF > ${WRKDIR}/json/${cohort_name}.SMNCNcalling.input.json
{	
  "SMNSampleCN.input_bams_samples": [ cat $bams ],
  "SMNSampleCN.input_bams_PON": "${WRKDIR}/SMNmodule/${cohort_name}_PON_bams",
  "SMNSampleCN.sample_list": [ cat $IDs ],
...
java -Dconfig.file=/home/credin/.cromwell_config.file -jar /mnt/raw-data/tools/cromwell/cromwell-86.jar run /mnt/raw-data/scripts/wdls/workflows/SMN-modules.wdl -i ${WRKDIR}/json/${cohort_name}.SMNCNcalling.input.json -o ${WRKDIR}/json/${cohort_name}.SMNCNcalling.options.json
